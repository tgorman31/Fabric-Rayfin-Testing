...
export function ProgrammeTimelineRow({ item, range, scale, dayWidth, placement, theme, onResizeStart, onMoveStart }: Props) {
  ...
  return <div ref={rowRef} className="relative h-21 overflow-hidden" style={{ width: range.totalDays * dayWidth }}>
    ...
    {placement ? <div className="absolute inset-0">
      {!item.startDate ? <div className={`absolute top-1/2 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rotate-45 ${theme.railClass}`} style={{ left: placement.left + dayWidth / 2 }} aria-label={`${item.rowLabel} milestone`} /> : <div className={`absolute top-1/2 h-11 -translate-y-1/2 rounded-full ${theme.railClass}`} style={{ left: placement.left, width: placement.width }}>
