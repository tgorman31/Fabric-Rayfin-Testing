...
import type { TargetPlanningDetailPatch, TargetProgrammeStageWorkspace } from "@/services/targetProgrammeService";
...
export function TargetProgrammeStageWorkspace({
  ...
  const timeline = useProgrammeTimeline({
    ...
    onCommit: (item, previous) => {
      ...
      if (Object.keys(patch).length > 0) onDatePatch(stage.code, item.definitionGuid, patch);
    },
  });
  ...
  return (
    <section className="mt-6 rounded-4xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">{stage.label}</p><h4 className="mt-2 text-xl font-semibold text-slate-950">{stage.label} Programme Workspace</h4></div>
      ...
      <div className="mb-5 grid gap-4 rounded-3xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-3">
        {stage.code === "ddtc" ? <label className="text-sm font-medium text-slate-700">Planning Status<select disabled={!stageEditable} value={workspace.ddtcDetail?.planning_status_code ?? ""} onChange={(event) => onPlanningStatus(event.target.value)} className="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm"><option value="">—</option><option>Not Lodged</option><option>Lodged</option><option>Granted</option></select></label> : null}
        {stage.code === "planning" ? <>
          <label className="text-sm font-medium text-slate-700">Advancing Gateway 4?<select disabled={!stageEditable} value={workspace.planningDetail?.advancing_gateway4_code ?? ""} onChange={(event) => onPlanningDetail({ advancingGateway4Code: event.target.value })} className="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm"><option value="">—</option><option>Yes</option><option>No</option><option>Yes (Partial)</option></select></label>
          <label className="text-sm font-medium text-slate-700">Planning Granted?<select disabled={!stageEditable} value={workspace.planningDetail?.planning_granted_code ?? ""} onChange={(event) => onPlanningDetail({ planningGrantedCode: event.target.value })} className="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm"><option value="">—</option><option>Yes</option><option>No</option></select></label>
          <label className="text-sm font-medium text-slate-700">Partial Advance G4: Name<input disabled={!stageEditable} value={workspace.planningDetail?.partial_advance_g4_name ?? ""} onChange={(event) => onPlanningDetail({ partialAdvanceG4Name: event.target.value })} className="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" /></label>
          <label className="text-sm font-medium text-slate-700">Partial Advance G4: # Homes<input type="number" min="0" step="1" disabled={!stageEditable} value={workspace.planningDetail?.partial_advance_g4_homes ?? ""} onChange={(event) => onPlanningDetail({ partialAdvanceG4Homes: event.target.value === "" ? null : Number(event.target.value) })} className="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" /></label>
        </> : null}
        <label className="text-sm font-medium text-slate-700">RAG<select disabled={!stageEditable} value={workspace.stageStatus?.rag_code ?? ""} onChange={(event) => onStatusPatch(stage.code, { ragCode: event.target.value })} className="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm"><option value="">—</option><option>R</option><option>A</option><option>G</option></select></label>
